...
was first introduced in the [magrittr](http://magrittr.tidyverse.org) package 
and later in in base R. The best way to
...
df <- tibble(Batting) |>
  ...
  mutate(teamID = as.character(teamID),
         teamID = ifelse(teamID == "FLO", "MIA", teamID)) |>
...
````
```{r, eval = FALSE}`r ''`
p <- ggplot(df, aes(x = yearID, y = team_hr, col = teamID))
...
```{r, eval = FALSE}
p <- ggplot(df, aes(x = yearID, y = team_hr))
...
p + geom_point() +
  ...
  labs(x = "Year",
       y = "Home Runs",
       title = "Home Runs by Year",
       subtitle = "Colorado Rockies and Florida Marlins"
  ) +
  guides(col = "none") +
  theme_bw()
```
...
3. After 2I import any necessary data and call that code chunk `data`. 
...
new_df <- new_df |>
  ...
  filter(yearID >= 1970)
p <- ggplot(new_df, aes(yearID, total_er, linetype = teamID))
p + geom_point() +
  geom_line() +
  scale_color_brewer(palette = "Set1") +
  labs(x = "year",
