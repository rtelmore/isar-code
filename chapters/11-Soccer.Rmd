...
Similar to hockey to some extent, soccer is notoriously low-scoring and there is 
...
p + geom_point(alpha = .5) +
  ...
  theme_bw()
```
One thing that we might want to do in plotting sports data is to identify the players
that we are plotting. In this case, we can use the `ggrepel` package to quickly 
add labels to certain points using the `label` aesthetic and the `geom_text_repel()`
function. Suppose we want to identify
players who are less than 20 years of age with a positive xG value, or have an xG 
