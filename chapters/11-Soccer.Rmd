...
statistical, or machine, learning umbrella of unsupervised learning. If you are
unfamiliar with statistical learning, in general, we recommend the excellent 
text *An Introduction to Statistical Learning with Applications in R* by James, 
...
```{r}
p <- ggplot(data = nwsl_cluster_df,
            aes(x = xGp90, y = xAp90, col = as.factor(k_means)))
p + geom_point() +
  scale_color_brewer("cluster", palette = "Dark2")
...
the makeup of the clusters. So, if we are wanting to identify similar players 
based on their scoring ability, let's only consider the appropriate metrics.
