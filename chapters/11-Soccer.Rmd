...
observations/clusters. We will use functions from the `factoextra` package to 
visualize the results of our clustering.
...
```{r}
# dend_plot <- factoextra::fviz_dend(hc)
...
```{r}
plot(hc)
rect.hclust(hc, k = 5, border = "forestgreen")
```
```{r}
fit
```
...
p <- ggplot(data = epl_player_stats_2022 |> 
              dplyr::mutate(new_player = ifelse(xG_xA_p90 > 1,
                                                player,
                                                NA)),
...
## Case Study
Something related to [Transfermarkt.com](https://www.transfermarkt.com). We will 
use the `worldfootballR` package to match player valuations (based on Transfermarkt)
with their statistics from the 2021-22 season in order to identify under- and 
over-paid players. 
...
```{r}
set.seed(8675)
epl_gk_kmean <- kmeans(dplyr::select(epl_gk_stats_2022, 
...
```{r}
epl_gk_stats_2022 |>dplyr::mutate(k_means = epl_gk_kmean$cluster) |> 
  dplyr::select(player, nation, k_means) |> 
  dplyr::arrange(k_means)
```
...
```{r}
epl_cluster_df <- dplyr::mutate(epl_gk_stats_2022, 
