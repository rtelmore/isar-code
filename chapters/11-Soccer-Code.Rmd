...
played (xGp90 and xAp90, respectively). We are going to consider three clusters 
initially. 
...
p + geom_point() +
  ...
  labs(x = "Age of Player",
       y = "Minutes Played") +
  guides(col = "none",
         pch = guide_legend(title = "cluster"))
...
versus expected goals and or assists? 
...
p + geom_point() +
  geom_point(data = data.frame(age = c(mix_model$mu[[1]][1], mix_model$mu[[2]][1]),
             ...
             aes(x = age, y = xGxAp90), col = "black", shape = 23, size = 3) +
  scale_color_grey("cluster") +
  guides(color = "none",
         pch = guide_legend(title = "cluster"))
```
...
p + geom_point() +
  ...
  scale_color_distiller("Probability", palette = "Greys", limits = c(0, 1)) +
  labs(color = "cluster",
       x = "Age",
