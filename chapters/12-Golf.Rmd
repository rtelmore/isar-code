...
Suppose Rory McIlory has probabilies of recording a one, two, three, four, five,
...
as $\pi_4^{R} = (0.0005, .2, .7, .1, .09, 0.0095)$ and 
$\pi_5^{R} = (0, 0.05, 0.35, 0.5, 0.0901, 0.0099)$. Similarly, Jordan Spieth's 
probabiliy vectors can be written as: (something with higher variance). Given that
...
```{r}
B <- 10000
rory_scores <- numeric(length = B)
for(b in 1:B){
  threes <- sum(apply(rmultinom(8, 
                                size = 1, 
                                prob = c(0.0001, 0.25, 0.6, 0.1, 0.04, 
                                         0.0099))*1:6,
                      2, sum))
  fours <- sum(apply(rmultinom(20, 
                               size = 1, 
                               prob = c(0.0005, .2, .7, .1, .09, 
                                        0.0095))*2:7,
                     2, sum))
  fives <- sum(apply(rmultinom(8, 
                               size = 1, 
                               prob = c(0, 0.05, 0.35, 0.5, 0.0901, 
                                        0.0099))*2:7,
                     2, sum))
  rory_scores[b] <- sum(threes) + sum(fours) + sum(fives)
}
```
