...
One question that you might have after looking at several of the previous figures 
is "How can we look at these figures as an animation?" Fortunately, the `gganimate` 
package makes turning a series of figures into an animation relatively easy. Let's take a look. Here are a few examples at looking at the top four shot takers for
the Denver Nuggets during the 2020-21 season. 

```{r,  eval = FALSE, include = T}
df <- readRDS("data/nuggets-2015-21.rds")
players <- c("Jamal Murray", "Nikola Jokic", "Will Barton", "Michael Porter Jr.")
ggplot(data = df %>% 
         dplyr::filter(., namePlayer %in% players, yearSeason == 2021),
       aes(x = -locationX, y = locationY, col = zoneBasic)) +
  ...
  scale_x_continuous("", breaks = NULL, limits = c(-250, 250)) +
  scale_y_continuous("", breaks = NULL, limits = c(-50, 420)) +
  transition_manual(namePlayer) + ### Here is where the animation starts
  labs(title = 'Player: {current_frame}')
```
...
ggplot(data = df %>% 
  ...
  scale_x_continuous("", breaks = NULL, limits = c(-250, 250)) +
  scale_y_continuous("", breaks = NULL, limits = c(-50, 420)) +
  transition_states(namePlayer) +
  labs(title = '2019-20 Shots for Player: {closest_state}')
```
...
Predict the probability of make/miss based on shot distance for Nikola Jokic 
during the 2021-22 season using a Generalized Linear Model. Filter the data to 
use only shots less than or equalto 30 feet.

```{r}
df <- readRDS("data/nuggets-2015-21.rds")
p <- ggplot(data = df %>% 
              dplyr::filter(., distanceShot <= 30, namePlayer == "Nikola Jokic") ,
            aes(x = distanceShot, y = ifelse(isShotMade, 1, 0)))
...
```
```{r}
mod_2 <- glm(isShotMade ~ poly(distanceShot, 2),
           data = df %>% 
              dplyr::filter(., distanceShot <= 30, namePlayer == "Nikola Jokic"),
           family = "binomial")
summary(mod_2)
